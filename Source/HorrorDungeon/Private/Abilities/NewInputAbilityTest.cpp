...
#include "Abilities/NewInputAbilityTest.h"

#include "AbilitySystemComponent.h"
#include "EnhancedInputComponent.h"
#include "GameFramework/Character.h"

UNewInputAbilityTest::UNewInputAbilityTest()
{
	// Sets the ability to default to Instanced Per Actor.
	InstancingPolicy = EGameplayAbilityInstancingPolicy::InstancedPerActor;	
}

void UNewInputAbilityTest::OnAvatarSet(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec)
{
...
void UNewInputAbilityTest::SetupEnhancedInputBindings(const FGameplayAbilityActorInfo* ActorInfo,
		...
		if (IsValid(ActorInfo->AvatarActor.Get()))
		{
			if (const APawn* AvatarPawn = Cast<APawn>(ActorInfo->AvatarActor.Get()))
			{
				if (const AController* PawnController = AvatarPawn->GetController())
				{
					...
					{
						if (UNewInputAbilityTest* AbilityInstance = Cast<UNewInputAbilityTest>(Spec.Ability.Get()))
						{
...
void UNewInputAbilityTest::HandleInputPressedEvent(const FGameplayAbilityActorInfo* ActorInfo,
...
void UNewInputAbilityTest::HandleInputReleasedEvent(const FGameplayAbilityActorInfo* ActorInfo,
	const FGameplayAbilitySpecHandle SpecHandle)
{
		// Find the Ability Spec based on the passed in information and set a reference.
		if (FGameplayAbilitySpec* Spec = ActorInfo->AbilitySystemComponent->FindAbilitySpecFromHandle(SpecHandle))
...
void UNewInputAbilityTest::OnRemoveAbility(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec)
{
		if (IsValid(ActorInfo->AvatarActor.Get()))
		{
			if (const APawn* AvatarPawn = Cast<APawn>(ActorInfo->AvatarActor.Get()))
